...
import { GUSTAVO_VERSION, isGustavoEnabled, queueGustavoInbound, processGustavoBatch, appendConfirmedMeetingLink } from './gustavo-sdr.js';
    ...
    try {
      for (const state of await fetchDueGustavoStates(5)) {
        ...
        await withConversationLock(lead.phone, async () => {
          await processGustavoBatch(state, (chatId, clientId, body) => sendBotText(client, chatId, clientId, body, [700,1400]));
        });
      }
    ...
    try {
      ...
      for (const item of queue) {
        try {
          ...
          if(item.body&&/Sua reuni[aã]o EC10[\s\S]{0,120}confirmada/i.test(item.body)) {
            const confirmedState=permission.clientState?await getBotConversationState(permission.clientState.phone):null;
            item.body=appendConfirmedMeetingLink(item.body,ec10GoogleMeetUrl,!!(confirmedState?.completed_at&&(confirmedState.metadata?.meeting as Record<string,unknown>|undefined)?.startsAt));
          }
