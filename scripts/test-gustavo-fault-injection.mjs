...
const primaryRouteSource = indexSource.match(/async function handleGustavoPrimaryRoute[\s\S]*?\r?\n}\r?\n\r?\ntype GustavoV2OracleResult/)?.[0] || "";
...
assert.match(aiSource, /\[404, 429, 500, 502, 503, 504\]\.includes\(response\.status\)/);
...
assert.match(indexSource, /for\(let attempt=0;attempt<1/);
assert.match(indexSource, /gustavo_v2_audio_recovery_queued/);
assert.match(indexSource, /gustavo_v2_poll_recovery_queued/);
