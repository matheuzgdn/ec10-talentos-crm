...
begin
 ...
 return jsonb_build_object('isOwner',is_owner,'timezone','America/Sao_Paulo','businessHours','Grade individual por vendedor · reuniões de 1 hora',
  'sellers',coalesce((select jsonb_agg(jsonb_build_object(
    'id',s.id,'name',coalesce(p.full_name,s.name),'defaultEnabled',a.enabled,'manualStartedAt',a.manual_started_at,
    'replaceOnManual',a.replace_on_manual,'services',a.services,
    'schedule',coalesce((select jsonb_agg(jsonb_build_object('weekday',w.iso_weekday,'startHour',w.start_hour,'endHour',w.end_hour) order by w.iso_weekday,w.start_hour)
      from whatsapp_bot.ec10_default_agenda_windows w where w.seller_id=s.id),'[]'::jsonb)))
  ...
  'slots',coalesce((select jsonb_agg(x.data order by x.starts_at) from (
   ...
   order by sl.starts_at limit 2500
  ) x),'[]'::jsonb));
end;
$$;
